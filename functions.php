...
add_action( 'bbp_theme_after_reply_author_details', 'nucommons_add_handle', 9 );
 ...
 *
 */
class NEUHeaderWalker extends Walker {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( ! empty( $icon_classes ) && ! empty( $args->theme_location ) ) {
			$item_output .= '<i ' . $archor_classes . '></i> <strong>' . $args->link_before . apply_filters( 'the_title', $item->title, $item->ID ) . $args->link_after . '</strong>';
		} else {
